...
Evaluates the quality of answers generated by the full RAG pipeline (retrieval + LLM generation), measured on 16–20 sampled cases using the [RAGAS](https://github.com/explodinggradients/ragas) evaluation framework.

| Metric | Score | Threshold | Status |
|---|---|---|---|
| **Faithfulness** | 0.433 | 0.80 | FAIL |
| **Answer Relevancy** | 0.733 | 0.80 | FAIL |

**Faithfulness** measures how factually grounded the generated answer is in the retrieved context (score range 0–1). The current score of 0.433 indicates that roughly 43% of the claims in generated answers can be directly attributed to the retrieved documents. The remaining claims appear to be drawn from the LLM's parametric knowledge rather than the retrieved context, which is a known challenge with financially-grounded RAG systems.

**Answer Relevancy** measures how well the answer addresses the question (score range 0–1). The score of 0.733 is below the 0.80 threshold, with some answers providing broader financial context than strictly required by the question.

Both metrics indicate room for improvement through prompt engineering, context filtering, and chunk quality refinement.
...
| Metric | Score | Pass Rate | Threshold | Status |
|---|---|---|---|---|
| **Tool Call Accuracy** | 0.950 | **95.0%** (19 / 20) | 0.70 | **PASS** |

The agent correctly routes 19 out of 20 queries to the appropriate tool. The one failure involves a query that requires both a live data lookup and a vector-store search simultaneously, which the agent handled with only the live tool.
...
|---|---|---|---|
| Similarity pass-rate — Chroma | 87.5% | 50% | ✅ PASS |
| Similarity pass-rate — OpenAI | 100.0% | 50% | ✅ PASS |
| Hit rate — News (Chroma) | 100.0% | 70% | ✅ PASS |
| Hit rate — News (OpenAI) | 100.0% | 70% | ✅ PASS |
| Hit rate — SEC Filings (Chroma) | 90.0% | 70% | ✅ PASS |
| Hit rate — SEC Filings (OpenAI) | 100.0% | 70% | ✅ PASS |
| Tool Call Accuracy | 95.0% | 70% | ✅ PASS |
| Response Time pass-rate | 95.2% | — | ✅ PASS |
| RAGAS Faithfulness | 43.3% | 80% | ❌ FAIL |
| RAGAS Answer Relevancy | 73.3% | 80% | ❌ FAIL |

Retrieval and tool-routing metrics all pass their quality thresholds. The generation-quality metrics (faithfulness and answer relevancy) are below threshold, driven by the LLM supplementing retrieved context with parametric knowledge.
...
| `TestRAGASEvaluation` | `live`, `rag` | **Faithfulness** | Calls the agent, collects retrieved context, evaluates with RAGAS `faithfulness` metric (LLM-as-judge: is each claim in the answer grounded in the retrieved context?). Model: `gpt-4o-mini`. Threshold: ≥ 0.80. |
| `TestRAGASEvaluation` | `live`, `rag` | **Answer Relevancy** | RAGAS `answer_relevancy` — embeds generated questions back and checks alignment with the original query. Threshold: ≥ 0.80. |
...
| `TestRAGASEvaluation` | `live`, `rag` | **Tool Call Accuracy** | Calls the agent for every RAG-annotated case; checks whether `expected_tool` (e.g. `get_news`, `search_sec_filings`) appears in the list of tool names called. Threshold: ≥ 0.70. |
