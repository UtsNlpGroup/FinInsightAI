...
Evaluates the quality of answers generated by the full RAG pipeline (retrieval + LLM generation), measured on 16–20 sampled cases using the [RAGAS](https://github.com/explodinggradients/ragas) evaluation framework. RAGAS uses `gpt-5.4-mini` as the judge LLM and `text-embedding-3-small` for embedding-based metrics.
...
All ten evaluated metrics pass their acceptance thresholds.
...
| `TestRAGASEvaluation` | `live`, `rag` | **Faithfulness** | Calls the agent, collects retrieved context, evaluates with RAGAS `faithfulness` metric (LLM-as-judge: is each claim in the answer grounded in the retrieved context?). Judge model: `gpt-5.4-mini`. Embedding model: `text-embedding-3-small`. Threshold: ≥ 0.40. |
